...
export default class TakeQuiz extends React.Component {
  ...
  selectAnswer = (ans, idx) => {
    ...
    questions.questions[this.state.activeQuestionIdx].answers[
      idx
    ].selected = true;
    let answers = this.state.answers;
    if (
      ans.name ===
      this.state.quiz.questions[this.state.activeQuestionIdx].correctAnswer
    ) {
      ...
      <>
        <div id="modal-wrapper-quiz" className="modal-wrapper-quiz">
          <div className="content">
            <div className="header">Testni yakunlashni hohlaysizmi ?</div>
            <div className="buttons-wrapper">
              <button onClick={this.hideModal}>Yo'q</button>
              <button onClick={this.finishQuiz}>Ha</button>
            </div>
          </div>
        </div>
          ...
          {this.state.authorized ? (
            <div className="content">
              <div className="header">
                <div className="left">{this.state.quiz.quizName}</div>
                <ProgressBar
                  className="center"
              ...
              <div className="body">
                <div className="left">
                  ...
                  <div className="question-bubble-wrapper">
                    {this.state.quiz.questions.map((ans, idx) => (
                      <span
                        onClick={() =>
                          this.setState({ activeQuestionIdx: idx })
                        }
                        key={idx}
                        className={
                          this.state.activeQuestionIdx === idx
                            ? "question-bubble selected-bubble"
                            : this.state.answers[idx] === 0
                            ? "question-bubble"
                ...
                <div className="right">
                  <div className="answers-wrapper">
                    {quiz.questions[activeQuestionIdx].answers.map(
                      (ans, idx) => (
                        <div
                          key={idx}
                          onClick={() => this.selectAnswer(ans, idx)}
                          className={
                            ans.selected === true ? "selected" : "answer"
                          }
                        >
                          {ans.name}
                        </div>
              ...
              <div className="footer">
                <div className="buttons-wrapper">
                  <button onClick={this.prevQuestion}>Oldingi</button>
                  {this.state.activeQuestionIdx + 1 <
                  this.state.quiz.questions.length ? (
                    <button onClick={this.nextQuestion}>Keyingi</button>
                  ) : (
                    <button onClick={this.showModal}>Yakunlash</button>
                  )}
                </div>
              </div>
            </div>
          ) : (
            <div>Ro'yhatdan o'tilmagan</div>
          )}
        </div>
      </>
    );
